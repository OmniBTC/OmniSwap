...
export default async function (hre: HardhatRuntimeEnvironment) {
    ...
    /// StargateFacet
    artifact = await deployer.loadArtifact("StargateFacet");
    contract = await deployer.deploy(artifact);
    const StargateFacet = contract.address;
    console.log(`${artifact.contractName} was deployed to ${StargateFacet}`);
    deployed["StargateFacet"] = StargateFacet
    writeFileSync(DEPLOYED, JSON.stringify(deployed, null, 4))
    ...
    /// LibSoFeeStargateV1
    const so_fee = 1e15
    const transfer_for_gas = 30000
    artifact = await deployer.loadArtifact("LibSoFeeStargateV1");
    contract = await deployer.deploy(artifact, [so_fee, transfer_for_gas]);
    const LibSoFeeStargateV1 = contract.address;
    console.log(`${artifact.contractName} was deployed to ${LibSoFeeStargateV1}`);
    deployed["LibSoFeeStargateV1"] = LibSoFeeStargateV1
    writeFileSync(DEPLOYED, JSON.stringify(deployed, null, 4))
