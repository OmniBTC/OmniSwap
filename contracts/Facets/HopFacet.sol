...
contract HopFacet is ISo, Swapper, ReentrancyGuard {
    /// Storage ///

    bytes32 internal constant NAMESPACE = hex"6d21be7f069eba22e6227bbf0972cf4a3ee2f0ce81ad8bd8004228e83b4830b8"; //keccak256("com.so.facets.hop");
    struct Storage {
    ...
    function startBridgeTokensViaHop(SoData calldata _soData, HopData calldata _hopData)
        external
        payable
        nonReentrant
    {
    ...
    function swapAndStartBridgeTokensViaHop(
        ...
        if (!LibAsset.isNativeAsset(address(_soData.sendingAssetId)) && msg.value != 0) revert NativeValueWithERC();
        _hopData.amount = _executeAndCheckSwaps(_soData, _swapData);
        _startBridge(_hopData);
