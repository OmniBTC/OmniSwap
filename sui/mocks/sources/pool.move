...
module omniswap_mock::pool {
    ...
    entry fun swap_token_x_<P, X, Y>(
        pool: &mut Pool<P, X, Y>, token: Coin<X>, min_amount: u64, ctx: &mut TxContext
    ) {
        transfer::public_transfer(
            swap_token_x(pool, token, min_amount, ctx),
    ...
    public fun swap_token_x<P, X, Y>(
        ...
        assert!(output_amount >= min_amount, EMIN_AMOUNT);
    ...
    entry fun swap_token_y_<P, X, Y>(
        pool: &mut Pool<P, X, Y>, token: Coin<Y>, min_amount: u64,ctx: &mut TxContext
    ) {
        transfer::public_transfer(
            swap_token_y(pool, token, min_amount, ctx),
            tx_context::sender(ctx)
        )
    }
...
module omniswap_mock::pool_tests {
    ...
    fun test_swap_sui_(test: &mut Scenario) {
        ...
        next_tx(test, the_guy);
        {
            ...
            let token = pool::swap_token_x(pool_mut, mint<TOKEN_X>(5000000, ctx(test)), 0, ctx(test));
    ...
    fun test_swap_tok_(test: &mut Scenario) {
        ...
        next_tx(test, owner);
        {
            ...
            let token = pool::swap_token_y(pool_mut, mint<TOKEN_Y>(1000, ctx(test)), 0, ctx(test));
