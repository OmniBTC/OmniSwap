module move_stl::skip_list {
    ...
    public fun destroy_empty<V: store + drop>(list: SkipList<V>) {
        ...
        assert!(table_with_length::length(&inner) == 0, ESkipListNotEmpty);
    ...
    public fun contains<V: store>(list: &SkipList<V>, score: u64): bool {
        table_with_length::contains(&list.inner, score)
    }
    ...
    public fun borrow<V: store>(list: &SkipList<V>, score: u64): &V {
        let node = table_with_length::borrow(&list.inner, score);
        &node.value
    }
    ...
    public fun borrow_mut<V: store>(list: &mut SkipList<V>, score: u64): &mut V {
        let node = table_with_length::borrow_mut(&mut list.inner, score);
        &mut node.value
    }
    ...
    public fun insert<V: store>(list: &mut SkipList<V>, score: u64, v: V) {
        assert!(!table_with_length::contains(&list.inner, score), ENodeAlreadyExist);
        ...
        while(l > 0) {
            ...
            while (is_some_and_lte(opt_next_score, score)) {
                let node = table_with_length::borrow_mut(&mut list.inner, option_u64::borrow(opt_next_score));
        ...
        table_with_length::add(&mut list.inner, score, new_node);
        if (is_some(&opt_l0_next_score)) {
            let next_node = table_with_length::borrow_mut(&mut list.inner, option_u64::borrow(&opt_l0_next_score));
    ...
    public fun remove<V: store>(list: &mut SkipList<V>, score: u64): V {
        assert!(table_with_length::contains(&list.inner, score), ENodeDoesNotExist);
        let (l, nexts) = (list.level, &mut list.head);
        let node = table_with_length::remove(&mut list.inner, score);
        while (l > 0) {
            let opt_next_score = vector::borrow_mut(nexts, l - 1);
            while (is_some_and_lte(opt_next_score, score)) {
                ...
                if (next_score == score) {
                    *opt_next_score = *vector::borrow(&node.nexts, l - 1);
                    ...
                    let node = table_with_length::borrow_mut(&mut list.inner, next_score);
                    nexts = &mut node.nexts;
                    opt_next_score = vector::borrow_mut(nexts, l - 1);
                }
            };
        ...
        while (l > 0) {
            ...
            while(is_some_and_lte(&opt_next_score, score)) {
                ...
                if (next_score == score) {
                    ...
                    let node = table_with_length::borrow(&list.inner, next_score);
    ...
    fun create_node<V: store>(list: &mut SkipList<V>, score: u64, value: V): (u64, Node<V>) {
        ...
        let level = rand_level(seed, list);
    ...
    fun print_skip_list<V: store>(list: &SkipList<V>) {
        ...
        while (is_some(next_score)) {
            let node = table_with_length::borrow(&list.inner, option_u64::borrow(next_score));
    ...
    fun check_skip_list<V: store>(list: &SkipList<V>) {
        if (list.level == 0) {
            assert!(table_with_length::length(&list.inner) == 0, 0);
            return
        };
        ...
        while (is_some(opt_next_score)) {
            let next_score = option_u64::borrow(opt_next_score);
            let next_node = table_with_length::borrow(&list.inner, next_score);
        ...
        assert!(size == table_with_length::length(&list.inner), 0);
        ...
        while (l > 0) {
            ...
            while(is_some(opt_next_0_score)) {
                ...
                let node = table_with_length::borrow(&list.inner, next_0_score);
    ...
    fun test_insert(
        ...
        timestamp::set_time_has_started_for_testing(apt);
        new_list_for_test<u256>(account, 16, 2, 1000, 1234, 0);
    }
    ...
    fun test_find(
        ...
        timestamp::set_time_has_started_for_testing(apt);
        new_list_for_test<u256>(account, 16, 2, 100, 12345, 0);
    ...
    fun test_remove(
        ...
        timestamp::set_time_has_started_for_testing(apt);
        new_list_for_test<u256>(account, 16, 2, 200, 5678, 0);
        let list = borrow_global_mut<SkipList<u256>>(signer::address_of(account));
        let scores = get_all_socres(list);
        ...
        while(n <= start) {
            ...
            let s2 = start + n;
            if (s1 >= 0) {
                remove(list, *vector::borrow(&scores, s1));
            };
            if (s2 != s1 && s2 < length ) {
                remove(list, *vector::borrow(&scores, s2));
            };
            n = n + 1;
        };
        check_skip_list(list);

        add_node_for_test(list, 300, 7890, 0);
        let scores = get_all_socres(list);
        let (n, length) = (0, vector::length(&scores));
        let skip = 0;
        while(n < length) {
            remove(list, *vector::borrow(&scores, n));
            skip = skip + 1;
            n = n + skip;
        };
        check_skip_list(list);
