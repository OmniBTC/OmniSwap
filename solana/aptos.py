...
from scripts.serde_struct import (
    omniswap_aptos_path,
    decode_hex_to_ascii,
    hex_str_to_vector_u8,
    change_network,
)
...
while True:
    try:
        package = aptos_brownie.AptosPackage(
            str(omniswap_aptos_path), network="aptos-mainnet"
        )
        break
    except:
        pass
...
def get_signed_vaa_by_wormhole(sequence: int, emitter_chain_id: str = None):
    ...
    url = (
        f"{wormhole_url}/v1/signed_vaa/{emitter_chain_id}/{emitter_address}/{sequence}"
    )
    ...
    try:
        ...
        if isinstance(result, list):
            ...
            return [
                v
                for v in result
                if str(v["dstWormholeChainId"]) == str(dstWormholeChainId)
            ]
        else:
            return []
    except:
...
def process_vaa(
    ...
    try:
        ...
        vaa_data, transfer_data, wormhole_data = parse_vaa_to_wormhole_payload(
            package, network.show_active(), vaa_str
        )
    ...
    try:
        ...
        if len(wormhole_data[3]) == 0:
            ...
            raise OverflowError
        local_logger.info(
            f"Execute emitterChainId:{emitterChainId}, sequence:{sequence}..."
        )
        if not is_admin:
            try:
        ...
        return False
    local_logger.info(
        f"Process emitterChainId:{emitterChainId}, sequence:{sequence} success!"
    )
    return True
    ...
    while True:
        try:
            pending_data = get_pending_data(
                url=pending_url, dstWormholeChainId=dstWormholeChainId
            )
            local_logger.info(f"Get signed vaa length: {len(pending_data)}")
        except Exception as e:
            local_logger.error(f"Get pending data for aptos error: {e}")
            continue
        ...
        for d in pending_data:
            try:
                ...
                local_logger.error(
                    f'Get signed vaa for :{d["srcWormholeChainId"]}, '
...
def main():
    ...
    t2 = threading.Thread(
        target=process_v2, args=(22, package.network_config["SoDiamond"])
    )
    t2.start()
    t2.join()
