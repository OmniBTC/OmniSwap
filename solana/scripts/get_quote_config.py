...
def get_bsc_test_quote_config(token_mint_in: str, ui_amount_in: str, network="devnet"):
    config = get_config(network)

    js_file_name = "get_bsc_test_quote_config.js"
    js_file_path = Path(__file__).parent.joinpath("test_dex").joinpath(js_file_name)

    new_env = {
        "ANCHOR_PROVIDER_URL": config["rpc_url"],
        "ANCHOR_WALLET": os.environ.get("TEST_REDEEM_PROXY"),
    }
