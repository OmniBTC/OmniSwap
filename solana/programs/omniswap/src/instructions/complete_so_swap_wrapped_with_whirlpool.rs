...
pub fn handler(
		...
		swap_by_whirlpool(&ctx, &swap_data)
	{
		...
		(other_proxy_recipient_account, ctx.accounts.recipient_token_account.as_ref(), 0, amount)
	} else {
		...
		(
			proxy_recipient_account,
			ctx.accounts.recipient_bridge_token_account.as_ref(),
			1,
			bridge_amount,
		)
	};
