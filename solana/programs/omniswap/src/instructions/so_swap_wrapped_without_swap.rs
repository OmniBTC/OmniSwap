...
use crate::cross::{ NormalizedWormholeData, NormalizedSoData };
use crate::message::SoSwapMessage;
use crate::state::sender_config::SenderConfig;
use crate::context::{ SoSwapWrappedWithoutSwap, SEED_PREFIX_BRIDGED};
...
pub fn handler(
    ...
    let normalized_wormhole_data = NormalizedWormholeData::decode_normalized_wormhole_data(
        &wormhole_data
    )?;
    let normalized_so_data = NormalizedSoData::decode_normalized_so_data(
        &so_data
    )?;
