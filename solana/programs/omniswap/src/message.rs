...
use super::cross::{NormalizedSoData, NormalizedSwapData};
use super::serde;
...
impl SoSwapMessage {
    ...
    pub fn decode_message(data: Vec<u8>) -> Result<Self, SoSwapError> {
        ...
        next_len = serde::deserialize_u8(&data[index..index + 1])? as usize;
        index = index + 1;
        let dst_max_gas = serde::deserialize_u256_with_hex_str(
            &data[index..index + next_len]
        )?;
        index = index + next_len;
        ...
        while index < data_len {
            ...
            swap_data.push(
                NormalizedSwapData::padding_swap_data(
                    swap_call_to,
...
impl AnchorDeserialize for SoSwapMessage {
    fn deserialize(buf: &mut &[u8]) -> io::Result<Self> {
        let so_swap_message = SoSwapMessage::decode_message(buf.to_vec())
            .unwrap_or_default();
